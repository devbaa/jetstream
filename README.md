...
`CustomerInvitationAccepted` now implements `ShouldDispatchAfterCommit`, so it is announced only once the outermost transaction commits and is dropped if that rolls back. Dispatching it after the controller's own transaction is not enough on its own: Laravel transactions nest, so where an application already has one open — transaction middleware, a larger workflow, a job that wraps its work — the controller's is a savepoint whose commit settles nothing. **If you listen for this event, it now fires later than it did**, at the end of the enclosing transaction rather than the middle of it; that is the point, since a listener firing earlier reads an account no other connection can see and may be told about an acceptance a later rollback takes away.
