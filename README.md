...
Without `--force`. **If your database already holds duplicate pending invitations, the migration deletes the extras, keeping the oldest of each set** — the index cannot be created over them, and deleting the row is what the application already does to an invitation when it is accepted or cancelled. The survivor names the same tenant, person and destination, so no invitation is lost; what is lost is the signed link carrying a deleted row's id, and an invitee holding that one needs to be invited again. Only rows with no customer account can be affected, since the index being replaced already separated every other pair. Run `select tenant_id, email, count(*) from customer_invitations where customer_account_id is null group by 1, 2 having count(*) > 1;` first if you want to see what will be collapsed.
