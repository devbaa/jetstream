...
| Effect | Existing credentials are revoked and the user can neither sign in nor stay signed in. | Target loses access & permissions where frozen. |
...
- **A blocked user cannot authenticate at all.** The rule — *a blocked user may neither establish nor resume stateful authentication* — is enforced at the authentication boundary rather than on any particular route, by listeners on Laravel's own lifecycle events: `Login` refuses a session being established (password, the second factor completing, a passkey, a remember cookie being redeemed), `Authenticated` refuses one that already existed, and `TwoFactorAuthenticationChallenged` refuses before the challenge is even presented. Refusing takes the state back — the guard's session key, the recaller, any half-finished `login.id` — without announcing a logout that never happened, and from the guard that actually holds it: the one named in the event, or `config('fortify.guard')` for the challenge, which carries no guard of its own. Routes of your own carrying only `auth` are covered; `account.active` remains as a backstop.
- This is built **beneath** Fortify rather than on `Fortify::authenticateUsing()` or `authenticateThrough()`. Those are single slots an application owns, and taking one would replace your authentication instead of composing with it. Your callback still runs and still decides whether the credentials are good; this only decides whether that user may come in.
- **`blocked_at` is what "blocked" means**, read directly. A user model of your own — `Jetstream::useUserModel()` — gets identical treatment; the previous `instanceof \App\Models\User` check quietly did nothing for one. `App\Models\User::isBlocked()` remains a convenience wrapper over the same column, and defining an `isBlocked()` of your own with different semantics does not change what the package enforces.
- **Scoped to the model Jetstream manages.** These are Laravel's authentication events, so they fire for every guard you have — but the check applies only to `Jetstream::userModel()`. An `App\Models\Admin` or a `Customer` you authenticate on a guard of your own is left alone, whatever it keeps in a column of that name. (Point `Jetstream::useUserModel()` and `auth.providers.*.model` at the same class, as the published configuration does; the boundary follows the former.)
- One behaviour change worth knowing in tests: `actingAs()` with an already-blocked user is now refused, because setting a blocked user as the current one is exactly what the boundary exists to prevent. Block *after* signing in instead.
...
**Three more events wait for the transaction that produced them.** `UserBlocked`, `DomainClaimVerified` and `DomainClaimSuperseded` were announced after their action's `DB::transaction()` had already returned. That is "after the commit" only when the action opened the outermost transaction: run inside one an application already had open, the action's own is a savepoint whose commit settles nothing, and — because the transaction manager holding a deferred event attaches the callback to whichever transaction was begun most recently across every connection — an unrelated connection's commit could announce a change that was not durable and might yet be rolled back.

All three now implement `ShouldDispatchAfterCommit` and are raised from inside their own transactions, so they are carried by them. Those transactions are also opened on the connection the state lives on — `$user->getConnection()` and `$claim->getConnection()` rather than the default one — because `useUserModel()` and `useDomainClaimModel()` let an application put either model elsewhere, and it is that connection's commit that decides whether the change happened.

`BlockUser`'s transaction covers the block itself. Personal access tokens and database sessions can be configured onto connections of their own, and revoking them is not part of it: the block is atomic, and the revocations that follow it are best-effort against whatever storage an application put them in. **If you listen for any of these, they now fire at the end of the enclosing transaction rather than the middle of it.** This is the same treatment `CustomerInvitationAccepted`, `TenantStaffUpdated` and `TeamMemberUpdated` already received. Events dispatched outside any transaction — the team and staff added/removed events, and the two announced deliberately *before* their work — are unchanged.

No action is needed on upgrade: all three are package code.
