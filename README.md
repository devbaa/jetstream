...
The invitation row is now taken with `lockForUpdate()` and held for the whole acceptance. Exactly one request consumes it and performs the account and membership changes; a second waits, then finds it consumed and gets the same 404 a spent invitation has always given, having written nothing. `CustomerInvitationAccepted` is dispatched after the commit rather than inside it, so a listener — a queue worker, anything not on that connection — can actually see the account it is told about.
